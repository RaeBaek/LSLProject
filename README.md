...
- 많은 경험을 할 수 있었던 프로젝트
    ...
    - 개발을 진행하며 전체 포스트에 대한 API는 있었으나 포스트 개별 API는 없었는데 게시물에 대한 detail 화면을 보여줘야 했던 나의 프로젝트에서는 개별 포스트에 대한 API가 필요하여 멘토 분들에게 **요청**을 드렸고 실제 현업에서도 백엔드 개발자와 API 요청 건으로 많은 대화를 나눈다고 말씀해 주시고 개별 포스트 API 또한 추가해 주셨다.
