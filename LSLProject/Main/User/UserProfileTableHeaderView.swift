...
final class UserProfileTableHeaderView: BaseTableViewHeaderFotterView {
    ...
    func setHeaderView(profile: MyProfile) {
        ...
        Observable.just(())
            .withUnretained(self)
            .flatMap { owner, _ in
                owner.repository.requestUserProfile(id: profile.id)
            }
            .withUnretained(self)
        ...
        Observable.just(())
            ...
            .subscribe(onNext: { owner, value in
                ...
                case .success(let data):
                    ...
                    // 내가 이미 팔로우 한 사람이라면?
                    // 팔로잉 처리!
                    if data.following.map({ $0.id }).contains(profile.id) {
        ...
        Observable.zip(userProfile, myProfile)
            .withUnretained(self)
            .bind { owner, value in
